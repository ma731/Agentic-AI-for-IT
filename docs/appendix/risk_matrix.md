...
| 6 | **Prompt injection / transcript poisoning** | Malicious input or a hallucinated tool/FOLLOWUP line hijacks behaviour or control flow | Tools are **pure functions over fixed scenario data** — no shell/eval, no open web; **routing can't be hijacked**: the next agent is chosen by the `_allowed_next()` policy (the LLM only orders an allow-listed set), FOLLOWUP targets are validated against `AGENT_NAMES` and capped by `MAX_VISITS`, and agents act via **drafts, never direct execution**; scope locked by `guardrails.md` on every prompt | Input sanitisation, allow-listed tool args, output filtering on the shared transcript |
