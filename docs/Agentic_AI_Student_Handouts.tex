...
\begin{itemize}
  ...
  \item \textbf{Case memory (across runs).} A persistent library of past incidents (predicted vs actual remaining life, the decision taken, and the outcome), queried during reasoning and grown automatically as runs finalize.
...
\subsection*{Learning}
A genuine recall-and-validate loop is implemented through the case memory:
\begin{itemize}
  \item \textbf{Recall.} During reasoning, the reliability agent retrieves the most similar past incidents (predicted vs actual remaining life, the decision taken, and the outcome), grounding its assessment in precedent rather than the live reading alone.
  \item \textbf{Append.} When a run finalizes, it is added to the case library with a ``pending'' outcome.
  \item \textbf{Reconcile.} On each new cycle, a reconciliation step closes any pending case once its actual failure time becomes known, computes whether the prediction landed in-window, and maintains a rolling RUL-accuracy percentage.
  \item \textbf{Audit and pattern matching.} Every tool call, agent report, routing decision, and human decision is logged for post-event review, and the RUL predictor matches current readings against the historical case set to calibrate confidence.
\end{itemize}
\textbf{Honest caveat for Q\&A:} the recall and reconciliation scaffolding is real and wired in, but the RUL predictor itself is a heuristic model, not a retrained ML model, and reconciliation depends on a maintenance/telemetry outcomes feed. True continuous retraining would close that feed to the live CMMS once each repair completes.
...
\begin{itemize}
  \item \textbf{Why not a single agent?} One agent calling 19 tools across five domains would produce a context too large to reason over reliably. Specialist separation keeps each agent's context focused.
  \item \textbf{Why not a deterministic pipeline?} The five challenges interact in ways that cannot be pre-scripted: the supply chain agent needs the exact part IDs from reliability, and the production agent needs to know which machines quality has cleared for extra load. A shared natural-language transcript lets agents adapt to each other's findings.
  \item \textbf{Why not pure peer-to-peer?} Without a supervisor, guaranteeing that compliance and safety runs before any action commits, and that the system terminates, requires explicit policy enforcement. The supervisor provides that guarantee while still letting the model decide ordering when policy allows it.
  \item The safety-HALT pattern, where one agent can stop the entire plan, is only expressible cleanly in a supervised topology where one node's output gates the final synthesis.
\end{itemize}
...
\begin{center}
\begin{tabularx}{\textwidth}{>{\raggedright\arraybackslash}X >{\raggedright\arraybackslash}p{4.6cm} >{\raggedright\arraybackslash}p{2.6cm}}
\toprule
...
\begin{itemize}
  \item \textbf{Never fabricate data:} a hard constraint in every agent prompt; if a tool fails, the agent must say so and stop, not estimate.
  \item \textbf{Low-confidence escalation:} the RUL predictor flags confidence below 0.6, and any agent seeing the flag escalates rather than planning.
  \item \textbf{Cost ceiling (\euro{}500):} the autonomy policy hard-caps autonomous cost commitments; anything above goes to APPROVE unconditionally.
  \item \textbf{Safety gate:} every proposed action is classified against the rules; a HALT stops the entire plan and routes to manual review.
  \item \textbf{Prompt-injection guard:} explicit instruction in all prompts to stop and flag if instructions appear embedded in tool outputs.
  \item \textbf{Token trimming:} transcript and routing digests are length-capped so context stays bounded.
  \item \textbf{Visit cap:} agent-to-agent follow-ups are capped at two visits per agent to prevent infinite loops.
  \item \textbf{Per-agent error handling:} each specialist runs inside a try/except, so a flaky tool or model error degrades gracefully and the run continues.
  \item \textbf{Self-evaluation:} each agent runs a per-domain checklist before finalizing (correct tool order, real sensor values used, stale data flagged).
\end{itemize}

\subsection*{Human-in-the-loop points}
\begin{enumerate}
  \item \textbf{Approval gate.} The graph pauses; the plant manager sees the supply summary, cost, and deadline, then approves or rejects; the checkpointer preserves state so the run resumes exactly where it paused.
  \item \textbf{ESCALATE path.} Interrupted or low-confidence telemetry routes directly to a deterministic ``insufficient data, human review required'' output with a checklist for the technician; no plan is generated.
  \item \textbf{Safety HALT.} Compliance and safety can stop the plan entirely, routing to the safety officer.
\end{enumerate}
