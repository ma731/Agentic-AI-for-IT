...
The agents share four context sources. The primary one is a shared transcript (a blackboard) to which every specialist appends its full natural-language report and which every later agent reads in full, complemented by a running latest-report summary for quick reference by the synthesizer. Within a run, a checkpointer provides episodic memory by persisting the full graph state across the approval pause, so the human decision resumes the exact same run. Across runs, a persistent case library of past incidents (which the reliability agent can recall, which grows as runs finalize, and whose predictions are validated as outcomes arrive) gives longer-term memory. Finally, an append-only audit trail records every tool call, agent report, routing decision, and human decision for post-incident reconstruction.

\subsection*{What changes over time vs what stays static}
What changes per run are the sensor readings, inventory levels, supplier lead times, job assignments, robot-cell state, maintenance-window availability, the run-specific audit trail, and the growing case library. What stays static within a scenario are the asset profiles (bill of materials, specs, equivalent machines), the safety rules referenced by the safety gate, the autonomy tiers, and the \euro{}500 cost ceiling.
...
The \textbf{APPROVE tier} covers any cost commitment above \euro{}500, an emergency maintenance window affecting a production commitment, any schedule change requiring crew reassignment, and the release of a work-order draft for execution. The \textbf{ESCALATE tier} covers any action touching plant safety systems and any safety shutdown that has occurred or is imminent (when \tool{safety\_gate} returns HALT, stopping the whole plan).
...
A deterministic policy enforces coverage and termination: reliability always runs first, on high risk all of supply chain, production, and quality must run before compliance and safety, compliance and safety always gates before finishing, and each agent is capped at two visits, so coverage is guaranteed regardless of the model's choices. The model-driven part is the ordering: when exactly one agent is allowed routing is deterministic with no model call, but when two or more are allowed (for example supply chain, production, and quality in any order) the supervisor chooses the sequence. Within each specialist, the ReAct agent then autonomously chooses which tools to call and in what order, looping until it has enough information.
